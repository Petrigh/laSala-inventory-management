...
export class StockLimpiezaComponent implements OnInit{
  addIcon = faPlus;
  buscarIcon = faSearch;
  editIcon = faPencil;

  listLimplieza: BienStock[] = [];
  ...
  ngOnInit(): void {
    ...
    this.stockService.getInsumos().subscribe((stock) => {
        this.listLimplieza = stock;
        this.datosTabla = stock;
    });
  }
  ...
  search(): void {
    this.visible = false;
    this.datosTabla = this.listLimplieza.filter((item: BienStock) => (item.nombre.toLocaleUpperCase().indexOf(this.searchValue.toLocaleUpperCase()) !== -1));
  }
  ...
  async onSubmit(edit: boolean, modal: FormStockComponent, modalRef: NzModalRef)
  {
    try {
      if (modal.validateForm.valid) {
        ...
        modalRef.close();
        this.listLimplieza = await lastValueFrom(this.stockService.getInsumos());
        this.datosTabla = this.listLimplieza;
      }else{ 
      ...
      console.error('Error updating Insumos de Limpieza', err);
    }
  }
  ...
  showModal(item: BienStock | undefined): void {
    ...
    modalRef.componentInstance.setType(Type.INSUMO);
  }
